...
function renderWorker() {
  ...
  shell(`<section class="screen stack">
    <div class="row"><div><h2>Open Shifts</h2><p class="muted">Filtered for ${state.user.role}, sorted by distance.</p></div><span class="chip">${state.user.role}</span></div>
...
function renderFacilityShifts() {
  ...
  shell(`<section class="screen stack"><h2>Posted Shifts</h2>${posted.map(s => `<article class="card"><div class="row"><h3>${s.role}</h3><span class="chip ${s.status === "claimed" ? "warn" : ""}">${s.status}</span></div><p class="muted">${dt(s.startTime)} - ${money(s.totalPayout)}</p><p class="muted">Claimed by: ${s.claimedBy || "Unclaimed"}</p></article>`).join("")}</section>`, tabs());
}

function renderAccount() {
  shell(`<section class="screen stack"><h2>Account</h2><div class="card"><h3>${state.user.name}</h3><p class="muted">${state.user.facilityType?.replaceAll("_", " ")}</p><p class="muted">${state.user.address}</p></div><div class="card"><h3>Billing</h3><p class="muted">Stripe account: Not connected</p></div><button class="btn secondary full" data-logout="1">Sign Out</button></section>`, tabs());
}
